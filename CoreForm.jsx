...
export default function CoreForm({ userId }) {
    ...
    const triggerAutomaticVerification = async () => {
        setLoading(true);
        setStatusMessage(
            '⚡ SÉCURITÉ : Déclenchement automatique. Analyse de la transaction sur la BSC...'
        );
        ...
        try {
            ...
            if (response.ok) {
                setStatusMessage(
                    '✅ PAIEMENT VALIDÉ ! Génération instantanée de vos liens achevée.'
                );
                setFinalLinks(data.links);
            } else {
                setStatusMessage(
                    `❌ REJET IMMÉDIAT : ${data.error || 'Vérification échouée.'}`
                );
            }
        } catch (err) {
            setStatusMessage(
                '❌ Erreur de connexion avec le serveur Point Focal.'
            );
        } finally {
            setLoading(false);
        }
    ...
    return (
        ...
        >
            ...
            >
                Remplissez les champs ci-dessous dans l'ordre requis.
                Dès que la troisième case sera complétée,
                le système interrogera directement la blockchain.
            </p>
            ...
            <div style={{ marginBottom: '25px' }}>
                ...
                >
                    🔐 Case 3 : Hash de la transaction
                    (Doit commencer par 0x et faire 66 caractères)
                </label>
